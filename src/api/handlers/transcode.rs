...
async fn get_active_encoder(pool: &SqlitePool) -> &'static EncoderConfig {
    ...
    // Get setting from DB
    let setting = SettingsService::new(pool.clone())
        .get_global("transcode_encoder")
        .await
        .unwrap_or(None);
...
pub async fn get_transcode_settings(
    ...
    let stored_pref = settings.get_global("transcode_encoder").await?;

    let current = stored_pref.unwrap_or_else(|| "Auto".to_string());

    let thread_count: Option<u32> = settings.get_global("transcode_threads")
        .await.unwrap_or(None).and_then(|v| v.parse().ok());

    let preset: Option<String> = settings.get_global("transcode_preset")
        .await.unwrap_or(None);

    let throttle_transcodes = settings.get_global("transcode_throttle")
        .await.unwrap_or(None).map(|v| v == "true").unwrap_or(false);

    let max_bitrate: Option<u32> = settings.get_global("transcode_bitrate")
        .await.unwrap_or(None).and_then(|v| v.parse().ok());
...
pub async fn update_transcode_settings(
    ...
    // Upsert encoder
    settings.upsert_global("transcode_encoder", &payload.encoder).await?;

    // Upsert threads
    let threads_val = payload.thread_count.map(|v| v.to_string()).unwrap_or_else(|| "0".to_string());
    settings.upsert_global("transcode_threads", &threads_val).await?;

    // Upsert preset
    let preset_val = payload.preset.unwrap_or_else(|| "".to_string());
    settings.upsert_global("transcode_preset", &preset_val).await?;

    // Upsert throttle
    let throttle_val = if payload.throttle_transcodes { "true" } else { "false" };
    settings.upsert_global("transcode_throttle", throttle_val).await?;
...
pub async fn get_hls_playlist(
    ...
    let resume_position = ProgressService::new(state.pool.clone()).resume_position(id).await;
